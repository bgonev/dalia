...
aws ec2 authorize-security-group-ingress --group-id $lb_sg --protocol tcp --port 5000 --cidr 0.0.0.0/0 >/dev/null
...
tg5k_arn=`aws elbv2 create-target-group --name web5ksrvrsc8 --protocol HTTP --port 5000 --vpc-id $vpc_id --query 'TargetGroups[0].TargetGroupArn' --output text`
